...
def main():
    ...
    # Profile the first N steps of the training loop to find bottlenecks
    N_PROFILE_STEPS = 30  # You can increase if you want a longer trace

    print(f"\n[Profiler] Profiling the first {N_PROFILE_STEPS} training steps to identify bottlenecks...\n")
    with profile(
        activities=[ProfilerActivity.CPU],  # Add ProfilerActivity.MPS if/when supported for Apple Silicon
        record_shapes=True,
        profile_memory=True
    ) as prof:
        ...
        for step in range(N_PROFILE_STEPS):
            ...
            with record_function("train_step"):
                ...
                optimizer.step()
                optimizer.zero_grad()
    print("\n[Profiler] Top 20 operations by CPU time:")
    print(prof.key_averages().table(sort_by="cpu_time_total", row_limit=20))
    print("\n[Profiler] Top 20 operations by memory usage:")
    print(prof.key_averages().table(sort_by="self_cpu_memory_usage", row_limit=20))
    print("\n[Profiler] Profiling complete. Exiting for analysis.\n")
    # Save the profiling results to a file for further analysis
    prof.export_chrome_trace(f"{logdir}/profile_trace.json")
    # You can visualize this trace using Chrome's tracing tool or other compatible tools.
