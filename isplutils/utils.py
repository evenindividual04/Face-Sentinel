    ...
    if face_policy == 'scale':
        ...
        if train:
            downsample_train_transformations = [
                A.Downscale(scale_factor_lower=0.5, scale_factor_upper=0.5, p=0.5),  # replaces scaled dataset
            ]
        else:
            downsample_train_transformations = []
    ...
    if train:
        aug_transformations = [
            A.Compose([
                ...
                A.OneOf([
                    A.ISONoise(),
                    A.GaussNoise(var_limit_min=10.0, var_limit_max=50.0, p=1.0),
                ]),
                A.Downscale(scale_factor_lower=0.7, scale_factor_upper=0.9, interpolation=cv2.INTER_LINEAR),
