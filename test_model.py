...
def main():
    ...
    parser.add_argument('--ffpp_faces_dir', type=str, action='store',
                        help='Path to the directory containing the faces extracted from the FF++ dataset. '
                             'Required for training/validating on the FF++ dataset.')
    parser.add_argument('--celebdf_faces_df_path', type=str, action='store',
                    help='Path to the Pandas Dataframe for Celeb-DF dataset')
    parser.add_argument('--celebdf_faces_dir', type=str, action='store',
                    help='Path to the directory containing faces for Celeb-DF dataset')
    ...
    parser.add_argument('--device', type=str, default='cuda:0' if torch.cuda.is_available() else 'cpu', help='Device (cuda:0, cpu, mps)')
    ...
    if args.device == 'mps' and mps_available():
    ...
    for dataset in test_sets:
        ...
        elif dataset == 'celebdf' and (args.celebdf_faces_df_path is None or args.celebdf_faces_dir is None):
            raise RuntimeError('Specify DataFrame and directory for Celeb-DF faces!')
    splits = split.make_splits(
    dfdc_df=dfdc_df_path, 
    ffpp_df=ffpp_df_path,
    celebdf_df=args.celebdf_faces_df_path,  # Add this line
    dfdc_dir=dfdc_faces_dir,
    ffpp_dir=ffpp_faces_dir,
    celebdf_dir=args.celebdf_faces_dir,     # Add this line
...
def process_dataset(df: pd.DataFrame,
    ...
    with torch.no_grad():
        ...
        for batch_data in tqdm(loader):
            ...
            batch_labels = batch_data[1].to(device, dtype=torch.float32)  # Force float32
